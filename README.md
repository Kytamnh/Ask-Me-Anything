...
1. **Request in**: The browser POSTs `{ history, newMessage }` to `/api/chat` (optional `debug: true` enables logs).
...
Logging is opt-in per request. Include `debug: true` in the request body to log classifier output, tool call arguments, and API key rotation. If omitted or false, logging is suppressed.
...
4. Update the profile data (used for personal Q&A):
   `data/profile.json`
5. (Optional) Replace `public/profile.jpeg` and update the contact line in `App.tsx`.
